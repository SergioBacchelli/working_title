...
## 9. BXF2 (минимальный профиль)

1. Источник истины
- Алгоритмы и правила формирования валидного минимального BXF2 находятся в:
  - `docs/specs/bxf2/BXF2_RULES.md`

2. Что считаем “готово” по BXF2
- Мы генерируем **только** минимально необходимые данные для модели:
  - панели (`parts/part` + `geometry(Box)/extent`)
  - положение панелей (`cabinets/.../partLinks/.../transformations`)
  - связку сцены и корпуса (`scene` → `cabinetLink`)
- Всё остальное (параметры, материалы, контейнеры и т.п.) добавляется **только если** появится подтверждённая необходимость.

3. Процесс изменения правил (обязательный)
- Сначала правим `docs/specs/bxf2/BXF2_RULES.md`.
- Затем проверяем валидность через импорт:
   - сравниваем с эталоном `examples/bxf2/Cleared*.bxf2`
   - генерируем `examples/bxf2/Generated*.bxf2` и импортируем (ручной регресс)
- Только после успешного импорта коммитим изменения.

4. Режим “не усложнять”
- Не добавляем вычислимые производные (зазоры/вкладной-накладной/bbox) в обязательную генерацию.
- Допускается считать их только как диагностику/валидацию вне BXF2.