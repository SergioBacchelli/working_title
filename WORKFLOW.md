...
## 9. Bazis Script (импорт панелей + отверстий)

Построение корпуса делаем через Bazis Script: портал считает, БАЗИС строит.

Источники истины:
- `docs/specs/bazis_script/README.md` — контракт JSON и правила импорта.

Как считаем изменение готовым:
1) Обновили контракт/правило в `docs/specs/bazis_script/README.md`.
2) Обновили (или добавили) эталонный JSON-пример.
3) Запустили импорт в БАЗИС и руками проверили, что:
   - панели созданы как панели,
   - размеры/ориентации/положение корректны,
   - отверстия присутствуют и имеют ожидаемые параметры.

Режим “не усложнять”:
- Сначала делаем минимальный корпус (5 панелей) + 1–2 типа отверстий.
- Всё остальное (пазы, кромка, фурнитура как объекты) — только после того, как PoC стабилен.